...
<body>
    ...
    <div class="z-1 position-fixed w-100 bottom-0 d-flex justify-content-end">
        <div class="border m-2 p-2 w-100 rounded bg-light">
            <div class="d-flex flex-row gap-2">
                ...
                $grab_count = mysqli_num_rows($view_grab); // Count the number of grab records
                ...
                ?>
                    <div class='position-relative '>
                        ...
                        <?php if ($is_user_in_east || $is_user_in_west || $is_user_in_lemery) { ?>
                ...
                <?php
                // Logic for displaying the depart or details button
                if ($is_user_in_east || $is_user_in_west || $is_user_in_lemery) {
                    echo "<a href='./depart.php?username=$user_username' onclick='return confirm(\"Are you sure you want to depart?\")' class='btn btn-success w-100'>Depart</a>";
                } elseif ($grab_count > 0) {
                    echo "<button popovertarget='detailbox' href='#' class='btn btn-success w-100'>Details</button>";
                } else {
                    echo "<a class='btn btn-secondary disabled w-100'>Depart</a>";
                }
                ?>
                <div id="detailbox" popover class="position-absolute rounded top-50 start-50 translate-middle px-3 py-3 border shadow-lg" style="width: 80%;">
                    <h4 class="text-center oswald mb-3">Depart and Payment details</h4>
                    <ul class="list-group">
                        <?php
                        mysqli_data_seek($view_grab, 0); // Reset the query pointer
                        while ($row = mysqli_fetch_assoc($view_grab)) { ?>

                            <div class="d-flex position-relative border gap-1 mb-1 fw-bold rounded shadow" style="height:60px;">
                                <div class="aspect-ratio1 p-1" style="width: 20%;">
                                    <img src="../Images/<?php echo $row['profile'] ?>" class="aspect-ratio2 rounded">
                                </div>
                                <div class="py-2 px-2" style="width: 40%;">
                                    <?php echo $row['username']; ?><br>
                                    <sup class="fw-normal">@<?php echo $row['username']; ?></sup>
                                </div>
                                ...
                                <a href="./remove_grab.php?id=<?php echo $id ?>" class="d-grid m-auto" onclick="return confirm('Are you sure you want to release this passenger?')" style=" width: 20%">
                                    <center>
                    ...
                    <button class="w-100 text-center mt-1 mb-0 btn text-primary" popovertarget="toption" onclick="return confirm('IMPORTANT! This is for Emergency only! Are you sure you want to transfer your passenger to other?')">Transfer Passenger?</button>
                    <a href="../driver/passenger.php" class="btn btn-success w-100 mt-2">Close</a>
                </div>
                <div id="toption" popover class="w-75 position-absolute rounded top-50 start-50 translate-middle p-4 border shadow-lg">
                    <h4 class="text-center oswald mb-3">Select where you want to transfer</h4>
                    <a class='btn btn-warning w-100 mb-2' href='./transfer.php?action=TransferEast' onclick='return confirm("Are you sure you want to transfer your passenger. It can not be undone?")'>Transfer to East</a>
                    <a class='btn btn-warning w-100 mb-2' href='./transfer.php?action=TransferWest' onclick='return confirm("Are you sure you want to transfer your passenger. It can not be undone?")'>Transfer to West</a>
                    <a class='btn btn-warning w-100 mb-2' href='./transfer.php?action=TransferLemery' onclick='return confirm("Are you sure you want to transfer your passenger. It can not be undone?")'>Transfer to Lemery</a>
                </div>
