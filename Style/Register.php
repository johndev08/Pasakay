...
<body>
    ...
    if (isset($_POST["register-passenger"])) {
        if (empty($_FILES['passenger_image']['name']) || empty($_POST["passenger_fname"]) || empty($_POST["passenger_age"]) || empty($_POST["passenger_gender"]) || empty($_POST["passenger_contact"]) || empty($_POST["passenger_street"]) || empty($_POST["passenger_username"]) || empty($_POST["passenger_password"])) {
            echo "<script>
            ...
            if (!in_array($imageExtension, ['jpg', 'jpeg', 'png'])) {
                ...
                if (mysqli_stmt_num_rows($stmt) > 0) {
                    ...
                    if (move_uploaded_file($tmpName, './Images/' . $newImageName)) {
                        ...
                        mysqli_stmt_bind_param($stmt, "sisssssss", $_POST["passenger_fname"], $_POST["passenger_age"], $_POST["passenger_gender"], $_POST["passenger_contact"], $_POST["passenger_street"], $username, $password, $newImageName, $access);
    ...
    if (isset($_POST["register-driver"])) {
        if (empty($_FILES['driver_image']['name']) || empty($_POST["driver_fname"]) || empty($_POST["driver_age"]) || empty($_POST["driver_gender"]) || empty($_POST["driver_contact"]) || empty($_POST["driver_permit"]) || empty($_POST["driver_username"]) || empty($_POST["driver_password"])) {
            echo "<script>
                    alert('Please fill in all required fields');
                </script>";
        } else {
            $fileName = $_FILES['driver_image']['name'];
            $fileSize = $_FILES['driver_image']['size'];
            $tmpName = $_FILES['driver_image']['tmp_name'];
            $imageExtension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
            ...
            if (!in_array($imageExtension, ['jpg', 'jpeg', 'png'])) {
                ...
                if (mysqli_stmt_num_rows($stmt) > 0) {
                    ...
                    if (move_uploaded_file($tmpName, './Images/' . $newImageName)) {
                        $stmt = mysqli_prepare($connection, "INSERT INTO users(fname, age, gender, contact, address_permit, username, password, profile, access) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        mysqli_stmt_bind_param($stmt, "sisssssss", $_POST["driver_fname"], $_POST["driver_age"], $_POST["driver_gender"], $_POST["driver_contact"], $_POST["driver_permit"], $username, $password, $newImageName, $access);
                        ...
                        if (mysqli_stmt_execute($stmt)) {
                            ...
                            echo "<script>alert('Error registering driver: " . mysqli_error($connection) . "');</script>";
                        }
                        mysqli_stmt_close($stmt);
                    } else {
                        echo "<script>alert('Failed to upload image');</script>";
                ...
                <div class="p-4">
                    <div>
                        <form method="POST" action="" enctype="multipart/form-data" class="p-4 rounded w-100 source-sans shadow">
                            ...
                            <label for="driver_permit" class="permit">Permit</label><br>
                            <div class="input-group mb-2">
                                <span class="input-group-text" id="basic-addon1">
                                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000">
                                ...
                                <input type="text" class="form-control" id="driver_permit" name="driver_permit" aria-describedby="basic-addon1" required>
                            </div>
