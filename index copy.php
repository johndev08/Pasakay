...
if (isset($_POST['login'])) {
    ...
    $password = $_POST['password']; //Will be handled by password_verify()

    $stmt = $connection->prepare("SELECT id, username, access, password FROM users WHERE username = ?");
        ...
        if (password_verify($password, $row['password'])) {
            ...
            switch ($access) {
                ...
                case 'passenger':
                    ...
                    break;
                default:
                    echo "<script>alert('Invalid user access level.');</script>";
            }
            exit; // Important to prevent further execution after header redirection
        } else {
